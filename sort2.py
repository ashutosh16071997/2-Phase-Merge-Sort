...
def sorting(argumentlist,order):
    ...
    if(tot*noofsplits>(mainmemory)):
        print("Error less memory")
        sys.exit(0)
    splittingfiles(inputfile,recordno,noofsplits,order,argumentlist,columnssize,columns,noofthreads)
    mergefiles(argumentlist,columnssize,columns,order,noofsplits)
    endtime = time.time()-start
    print(endtime)
    print("#########################EXECUTION TERMINATED#######################")
def splitthread(cn,order,values,intermediatefile):
...
def writelast(intermediatefile,values,argumentlist,order,columns):
    ...
    values.clear()
    f11.close()                
def splittingfiles(inputfile,recordno,noofsplits,order,argumentlist,columnsize,columns,noofthreads):
    ...
    for line in f:
        ...
        if(record == recordno):
            ...
            for val in fn:
               cn.append(columns.index(val))
            with ThreadPoolExecutor(max_workers=int(noofthreads)) as threadexecution:
                threadexecution.submit(splitthread,cn,order,values,intermediatefile)   
            #ff = lambda x:[x[i] for i in cn]
            #if order == "asec":
            #    print("Sorting files")
            #    values.sort(key = ff)
